...
DontGetSNPpyWithMe.py builds a multi-task linear neural network. It is designed to take in 
...
def PhotoCopierInitialize(laya, default = "xavier"):
...
class DiddyKongRacing(nn.Module):
    ...
    def __init__(self, inputDimensions = [2,512], numLayers = 3, layerWidths = None, dropout = 0.5, multitaskOutputs = [1,1]):
...
class DonkeyKong64(nn.Module): 
    ...
    def __init__(self, diddyKong, donkeyKong, krool, itsTheFinalCountdown): #(self, input, output, dropout, isThisTheFinalLayerForMultiTask)
